...
plot_statistic_dynamic <- function(stat_name, y_label = stat_name, show_legend = FALSE, fixed_y = TRUE) {
 ...
 for (i in 1:nrow(params)) {
  ...
  c_val  <- if("Param_c" %in% names(params)) params$Param_c[i] else NA
  ...
  pred_smooth <- switch(m_name,
                        ...
                        "PowerDecay_3p" = H_PowerDecay(k_smooth, a_val, b_val, c_val),
 ...
 tryCatch({
  ...
  for (i in seq_len(nrow(optimized_parameters_df))) {
   ...
   predicted_vals <- switch(model_type,
                            ...
                            "PowerDecay_3p" = H_PowerDecay(k_val, a_val, b_val, c_val),
                            rep(NA, length(k_val)))
  ...
  for (stat in unique(optimized_parameters_df$Statistic)) {
   ...
   for (i in seq_len(nrow(params))) {
    ...
    predicted_vals <- switch(m_name,
                             ...
                             "PowerDecay_3p" = H_PowerDecay(k_predict, a_val, b_val, c_val),
  ...
  # 3. Dynamic Y-Axis Plots for L-Moments
  plot_l1_dyn <- plot_statistic_dynamic("l1_pos", y_label = expression(L[1]^{(k)}), fixed_y = FALSE)
  plot_l2_dyn <- plot_statistic_dynamic("l2_pos", y_label = expression(L[2]^{(k)}), fixed_y = FALSE)
  plot_l3_dyn <- plot_statistic_dynamic("l3_pos", y_label = expression(L[3]^{(k)}), fixed_y = FALSE)
  plot_l4_dyn <- plot_statistic_dynamic("l4_pos", y_label = expression(L[4]^{(k)}), fixed_y = FALSE)
  ...
  qq_l1 <- plot_qq_dynamic("l1_pos", y_label = expression(L[1]^{(k)}))
  qq_l2 <- plot_qq_dynamic("l2_pos", y_label = expression(L[2]^{(k)}))
  qq_l3 <- plot_qq_dynamic("l3_pos", y_label = expression(L[3]^{(k)}))
  qq_l4 <- plot_qq_dynamic("l4_pos", y_label = expression(L[4]^{(k)}))
  
  qq_l_list <- list(qq_l1, qq_l2, qq_l3, qq_l4)
