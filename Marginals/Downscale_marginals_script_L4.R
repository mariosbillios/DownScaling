...
library(ggplot2)
library(tidyr)
library(VGAM) # Required for the Hurwitz zeta function

# =====================================================================
# GLOBALS & MODEL DEFINITIONS
# =====================================================================

global_expected_models <- c("Truncated_Zeta_Mixture")
global_expected_data   <- c("Calib. data", "Valid. data")

# 2-Component Truncated Zeta Mixture Function
H_TZ_Mixture <- function(k, alpha, w1, l1 = 1, l2 = 2) {
  # alpha must be > 1 for the Hurwitz zeta function to converge
  if(alpha <= 1) return(rep(Inf, length(k)))
  
  w2 <- 1 - w1
  
  # Calculate the Hurwitz zeta normalizers
  zeta_1 <- VGAM::zeta(x = alpha, shift = l1)
  zeta_2 <- VGAM::zeta(x = alpha, shift = l2)
  
  # Calculate the mixture components
  comp_1 <- ifelse(k >= l1, (1 / zeta_1) * (k^(-alpha)), 0)
  comp_2 <- ifelse(k >= l2, (1 / zeta_2) * (k^(-alpha)), 0)
  
  # Return the combined prediction
  return((w1 * comp_1) + (w2 * comp_2))
}

mse_TZ_Mixture <- function(par, k, y) {
  predictions <- H_TZ_Mixture(k, alpha = par[1], w1 = par[2])
  mean((y - predictions)^2, na.rm = TRUE)
}
...
plot_statistic_dynamic <- function(stat_name, y_label = stat_name, show_legend = FALSE, fixed_y = TRUE) {
  ...
  for (i in seq_len(nrow(params))) {
    ...
    a_val  <- params$Param_a[i] # alpha
    b_val  <- params$Param_b[i] # w1
    
    pred_smooth <- H_TZ_Mixture(k_smooth, alpha = a_val, w1 = b_val)
  ...
  line_colors <- c("Truncated_Zeta_Mixture" = "blue")
  line_types  <- c("Truncated_Zeta_Mixture" = "solid")
  ...
  line_guide  <- if(show_legend) guide_legend(order = 2, nrow = 1) else "none"
...
for (current_station_file in station_files) {
  ...
  tryCatch({
    ...
    for (scale_name in names(clean_bins_list)) {
      ...
      # Retaining lmomco computation to extract L-moment ratios
      if (length(x_pos) >= 4) {
        lm_pos <- lmoms(x_pos)
        t2_pos <- lm_pos$ratios[2]
        t3_pos <- lm_pos$ratios[3]
        t4_pos <- lm_pos$ratios[4]
      } else { t2_pos <- t3_pos <- t4_pos <- NA }
      
      scale_summary <- data.frame(
        scale = scale_name, t2_pos = t2_pos, t3_pos = t3_pos, t4_pos = t4_pos
      )
    ...
    # Targeting ONLY the L-moment ratios as requested
    stats_to_fit <- c("t2_pos", "t3_pos", "t4_pos")
    ...
    for (stat in stats_to_fit) {
      ...
      # Optimize alpha (bound 1.01 to 10) and w1 (bound 0 to 1)
      fit_TZM <- DEoptim(mse_TZ_Mixture, 
                         lower = c(1.01, 0), 
                         upper = c(10.0, 1), 
      ...
      res_row <- data.frame(
        Statistic = stat, 
        Model = "Truncated_Zeta_Mixture", 
        H0 = NA,
        Param_a = fit_TZM$optim$bestmem[1], # alpha
        Param_b = fit_TZM$optim$bestmem[2], # w1
        Param_c = NA, 
        MSE = fit_TZM$optim$bestval
      )
    ...
    for (i in seq_len(nrow(optimized_parameters_df))) {
      ...
      a_val <- current_fit$Param_a # alpha
      b_val <- current_fit$Param_b # w1
      ...
      predicted_vals <- H_TZ_Mixture(k_val, alpha = a_val, w1 = b_val)
    ...
    for (stat in unique(optimized_parameters_df$Statistic)) {
      ...
      for (i in seq_len(nrow(params))) {
        ...
        predicted_vals <- H_TZ_Mixture(k_predict, alpha = a_val, w1 = b_val)
    ...
    # 1. Fixed Y-Axis Plots (Only L-Moment ratios)
    plot_t2_fixed <- plot_statistic_dynamic("t2_pos", y_label = expression(t[2]^{(k)}), show_legend = TRUE, fixed_y = TRUE)
    ...
    if (!is.null(plot_t2_fixed)) {
      combined_plot_fixed <- plot_t2_fixed / (plot_t3_fixed | plot_t4_fixed) + 
    ...
    plot_t2_dyn <- plot_statistic_dynamic("t2_pos", y_label = expression(t[2]^{(k)}), show_legend = TRUE, fixed_y = FALSE)
    ...
    if (!is.null(plot_t2_dyn)) {
      combined_plot_dynamic <- plot_t2_dyn / (plot_t3_dyn | plot_t4_dyn) + 
        plot_layout(guides = "collect") & theme(legend.position = "bottom", legend.box = "horizontal", legend.box.just = "left", legend.spacing.x = unit(0.5, "cm"))
    ...
    qq_list <- list(qq_t2, qq_t3, qq_t4)
